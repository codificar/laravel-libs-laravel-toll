		...
		fetch(page = 1,tollCategory_filter) {
			...
			tollCategory.search(component.searchParams)
				.then((response) => {
					...
					component.tollsCategory = response.data.toll;
				});
		},
						...
						<table class="table">
							...
							<tbody>
								<tr v-for="tollCategory in tollsCategory.data" v-bind:key="tollCategory.id">
									<td>{{ tollCategory.id }}</td>
									<td>{{ tollCategory.name }}</td>
									<td style="width: 100px;">
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-center" role="menu" aria-labelledby="dropdownMenu1">
												 <!-- EDITAR -->
												<button v-if="EditPermission" class="dropdown-item" tabindex="-1" id="show-modal-edit" @click="modal(true,true,tollCategory)" type="button">
													{{trans('toll.edit')}} 
												</button>

												 <!-- DELETAR -->
												<button v-if="DeletePermission"  type="button" class="dropdown-item" v-on:click="deleteTollCategory(tollCategory.id, tollCategory.name)" tabindex="-1" >
													{{trans('toll.delete')}}
												</button>
											</div>
										</div>
